...
export default () => (
  <Post
    ...
    <p>
      I love this about Python - it leaves you with enough power to make something happen in multiple ways, but it has one <i>obvious</i> or <i>idiomatic</i> (dare I say <i>Pythonic</i>?) way to do it.
    </p>
    ...
    <p>
      String formatting is one of those things where you need to do it alot, so you'll learn it one way. C has <code>`printf()`</code> for this. And it works great.
    </p>

    <p>
      Most languages have some means of string formatting similar to <code>`printf()`</code>. Some go the extra mile and add my favorite style of String Interpolation, where you can evaluate expressions directly inside of the string (and not simply sprinkle them in later, as you do with <code>`printf()`</code>). In Ruby, this is done like so:
    </p>
...
"String #{op}"`}
    ...
    <p>
      This is my favorite syntax for string formatting, and I desperately wish Python had something similar. Thankfully, <a href="https://www.python.org/dev/peps/pep-0498/">we're getting this in Python 3.6 with <i>f-strings</i> from PEP 498</a>!
    </p>

    <h2>f-strings</h2>

    <p>
      f-strings are the <i>new</i> new style of string formatting presented in PEP 498. They're similar to Ruby's string interpolation shown above, and basically provide this syntax:
    </p>
    ...
    <p>
      As you can see, they're very easy to use - simply slap an <code>`f`</code> in front of the string literal and type your expressions inside of <code>`{}`</code>.
    </p>
    ...
    <p>
      Why do I want Python 3.6 so badly? Because I feel we're sorely lacking in the syntactical sugar department for current string interpolation options. Python is a very terse language - that is, it avoids excessive verbosity (unlike my blog). This, to me, feels very <i>un</i>-Pythonic.
    </p>
...
print("Hi, my name is %, and I love %." % (name, thing))`}
    ...
    <p>
      The biggest pitfall to this operator is that it only supports a few types - only <code>`ints`</code>, <code>`strs`</code>, and <code>`doubles`</code> can be formatted. All other types must be converted to one of these or are simply not supported. So you see it's not particularly powerful, but it is fast.
    </p>

    <p>
      However, this operator also has some finnicky behavior when a tuple is passed in. For instance, if <code>`name`</code> was a tuple, the snippet below would present this error:
    </p>
...
TypeError: not all arguments converted during string formatting`}
    ...
    <p>
      <a href="https://docs.python.org/3/library/string.html#formatstrings">View the docs</a>
    </p>

    <p>
      Python also has what the gurus might call "new-style string formatting" with the <code>`str.format()`</code> function.
    </p>

    <p>
      <code>`str.format`</code> alleviates the inconsistency between supplying only one versus more arguments. Additionally, to quote PEP 498:
    </p>

    <blockquote>
      In particular, it uses normal function call syntax (and therefor supports multiple parameters) and it is extensible through the <code>`__format__()`</code> method on the object being converted to a string.
    </blockquote>

    <p>
      Here's an example of <code>`str.format()`</code> using keys to map arguments to their positions in the string:
    </p>
    ...
    <p>
      <a href="https://docs.python.org/3/library/string.html#template-strings">View the docs</a>
    </p>

    <p>
      <code>`str.Template()`</code> takes a different approach. Let's take a peek first:
    </p>
...
print(t.substitute(name="Josh", thing="programming languages"))`}
    ...
    <p>
      As you can see, this will suffer from some of the same issue that <code>`str.format()`</code> does, with separating the arguments from their string and its verbosity.
    </p>
...
print(output.substitute(a=template_times[0], b=template_times[1], c=template_times[2]))`}
    ...
    <p>
      <i>For what it's worth, this experiment was conducted using Python 2.7 on Windows 10 with an i5-4690K @ 3.50 GHz.</i>
    </p>
    ...
    <p>
      Should you need formatting capabilities and types supported, <code>`str.format()`</code> is still a good choice.
    </p>

    <p>
      Now you might wonder where <code>`str.Template()`</code> is useful then. If it's slower than we first thought, and extremely so, why use it?
    </p>
    ...
    <p>
      Either way, I'm itching to get my hands on Python 3.6 - I <i>love</i> the new syntax for f-strings, and can't wait to benchmark it against the older styles!
    </p>

    <p>
      <i>(Is this how the programmers waiting for <code>`str.Template()`</code> felt, potentially unaware of the performance costs awaiting them?)</i>
    </p>
  </Post>
)
